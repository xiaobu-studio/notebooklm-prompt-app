...
當我提供【簡報主題】與【受眾】時，請你「直接輸出」以下內容，不需要任何開場白。請嚴格依照以下格式輸出：

### 🏷️ 建議聊天室主題：(請根據我的需求，生成一句 10 字以內、精準俐落的標題，方便我整理對話紀錄)
...
3. 預期資料目標：(明確指示 NotebookLM 在網路上需尋找並整理出哪些具體的工具、數據指標或實際使用情境)
*注意：這是一段「網路搜尋與資料篩選指令」，請勿要求 NotebookLM 憑空生成長篇文章，而是要求它「搜尋並篩選出符合上述條件的最佳來源資料」。
...
1. 簡報設計師角色設定與受眾語氣對焦。
2. 任務目標：請嚴格「僅根據左側的資料源」來萃取重點。
3. 視覺與排版強制要求 (請務必將以下兩項條件「原封不動」寫入提示詞中)：
...
4. 輸出格式：請輸出 5 頁簡報大綱。每頁需包含標題與 3 個核心重點。`
        });
